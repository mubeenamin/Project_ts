...
let userStoredId: string;
let userStoredPin: number;
...
async function userProcess() {
  ...
  do {
    ...
    if (userData.userID === userStoredId && Userpin === userStoredPin) {
      ...
      console.log(
        chalk.bold.red("Your User ID and pin is not correct try again!")
      );
    }
...
async function mainMenu() {
  async function balance() {
    if (count > 0) {
      console.log(
        "Your Total Balance is: Rs " + chalk.bold.blue(remainingBalance)
      );
    } else {
  ...
  async function fundTransfer() {
    ...
    spiner.success({
      text:
        "Fund has been sent to " + chalk.bold.green(fundTransferMenu.recipient),
    });
    count += 1;
